...
all_models = {'llama': ['meta/meta-llama-3.1-405b-instruct', 'meta/meta-llama-3-70b-instruct', 'meta/meta-llama-3-8b-instruct', 'meta/llama-2-7b-chat', 'meta/llama-2-70b-chat', 'meta/llama-2-13b-chat'],
          ...
          'claude': ['claude-3-5-sonnet-20240620', 'claude-3-sonnet-20240229', 'claude-3-haiku-20240307', 'claude-2.0', 'claude-2.1', 'claude-instant-1.2']} # 'claude-3-opus-20240229']}
...
def main():
    ...
    for model in all_models['llama'] + all_models['gpt'] + all_models['claude']:
        ...
        df[f"{model}_mean"] = df[model_cols].append(lambda x: [x[col] for col in model_cols if x[col] is not None]).mean(axis=1)
        df[f"{model}_score"] = df[[f"{model}_{i}" for i in range(1)]].apply(lambda x: 6-x if df['reversed'] == True else x, axis=1)
